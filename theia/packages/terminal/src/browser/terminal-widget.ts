...
export class TerminalWidget extends BaseWidget implements StatefulWidget {
    ...
    constructor(
        ...
        this.userId = getParameterByName('uid');
        this.environment = getParameterByName('env');
    ...
    storeState(): object {
        ...
        return { terminalId: this.terminalId, titleLabel: this.title.label, userId: this.userId, environment: this.environment };
    }

    restoreState(oldState: object) {
        if (this.restored === false) {
            const state = oldState as { terminalId: number, titleLabel: string, userId: string, environment: string };
            /* This is a workaround to issue #879 */
            this.restored = true;
            this.title.label = state.titleLabel;
            if (state.userId === this.userId && state.environment === this.environment) {
    ...
    public async start(id?: number): Promise<void> {
        ...
        if (id === undefined) {
            ...
            this.terminalId = await this.shellTerminalServer.create(
                { rootURI, cols: this.cols, rows: this.rows, userId: this.userId, environment: this.environment });
        } else {
