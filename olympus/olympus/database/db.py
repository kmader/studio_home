...
DB_PATH = os.path.join(os.environ.get("DEPLOY_DIR"), 'olympus_db.json')
TABLE = 'models'
...
def update_model(name, version, updates):
	for update_key, update_val in list(updates.items()):
...
def get_models_by_name(name):
	db = TinyDB(DB_PATH)
	return db.table(TABLE).search(Model.name == name)
