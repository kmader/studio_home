	...
	def parse_input(self, input):
		...
		if isinstance(input, list):
			if num_inputs == 1:
				input = input[0]
				try:
					input = self.format_input(input, list(self.model_obj['input_spec'].values())[0]['shape'], 
						list(self.model_obj['input_spec'].values())[0]['dtype'])
				except Exception as e:
			...
			if num_inputs == 1:
				try:
					input = self.format_input(list(input.values())[0], list(self.model_obj['input_spec'].values())[0]['shape'], 
						list(self.model_obj['input_spec'].values())[0]['dtype'])
				except Exception as e:
					return (False, self.return_error('Input error: ' + str(e)))
			else:
